         ...
         std::string &v)
{
  ...
  for(int i = 0; i < size; ++i){
    ...
    if(rc){
      ...
      int ret =rc->call(paxos_protocol::preparereq, me, a, r, rpcc::to(1000));
        ...
        std::vector<std::string> nodes, std::string v)
{
  ...
  for(int i = 0; i < size; ++i){
    ...
    if(rc){
      bool r = false;
      int ret =rc->call(paxos_protocol::acceptreq, me, a, r, rpcc::to(1000));
	      ...
	      std::string v)
{
  ...
  for(int i = 0; i < size; ++i){
    ...
    if(rc){
      int r = 0;
      int ret =rc->call(paxos_protocol::decidereq, me, a, r, rpcc::to(1000));
    }
  }
}
...
acceptor::acceptreq(std::string src, paxos_protocol::acceptarg a, bool &r)
{
  ...
  r = false;
  ScopedLock ml(&pxs_mutex);
  if(a.instance == (instance_h+1) && a.n.n >= n_h.n){
    n_a = a.n;
    v_a = a.v;
    l->logaccept(n_a, v_a);
    r = true;
...
acceptor::commit_wo(unsigned instance, std::string value)
{
  ...
  if (instance > instance_h) {
    tprintf("commit: highest accepte instance = %d\n", instance);
